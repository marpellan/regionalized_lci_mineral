### --- From Besseau et al (2023) --- ###

# A usefull function do display inventory of datasets
def display_inventory(act, index_name_short=False):
    print(act)
    df = pd.DataFrame(index=['amount', 'unit', 'key', 'type'])
    for (i, exc) in enumerate(act.exchanges()):
        if index_name_short:
            # print(exc.input.as_dict())
            index_name = exc.input['name']
        else:
            index_name = str(bw.get_activity(exc.input.key))

        if index_name in df.columns:
            df[index_name].loc['amount'] += exc.amount
        else:
            df[index_name] = [exc.amount,
                              exc.unit,
                              exc.input.key,
                              exc['type']]
    return df.T
...
def lcia_system_contribution(lca, activity, skip_inventories, lcia_methods, CONTRIBUTORS_LIST, breakdown_lists,
                             activity_amount=1):
    '''
    ...
    for impact in process_emissions_impact:
        ...
        # and 97% to "Process emissions"
            ...
            "name"] == "nickel matte production, nickel-cobalt sulphite and nickel sub materials, mass allocation":
